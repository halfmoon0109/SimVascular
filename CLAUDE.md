...
## 에이전트 간 인수인계 규칙
- 작업의 **목적·제약·폐기한 시도·열린 문제**는 `docs/wall-mesh-purpose.md`에 있다. 작업 시작 시 가장 먼저 읽고, 처방을 바꾸거나 접거나 새 결함을 찾으면 같은 커밋에서 갱신한다. 이 문서는 커밋되므로 웹 세션·Codex·사용자 모두가 본다.
- 같은 브랜치를 여러 세션(Mac의 Claude Code·Codex, 웹의 Claude)이 번갈아 쓴다. **작업 시작 전 `git pull --ff-only origin <브랜치>`, 작업을 마치면 반드시 `git push`.** 한 세션이 푸시하기 전에는 다른 세션에서 작업을 시작하지 않는다.
- `handoff/`는 Mac 로컬 전용이라 웹 세션은 읽을 수 없다. 웹 세션이 알아야 할 결론(왜 이렇게 했는지, 다음에 무엇을 볼지)은 커밋 메시지에 담는다.
...
## 에이전트 간 소통 문서
- `handoff/YYYY-MM-DD.md`에 날짜별로 피드백을 주고받는다. 형식·규칙은 `handoff/README.md`.
- 작업 시작 시 `logs/` 최신 로그 다음으로 **`handoff/`의 가장 최근 파일**을 읽고, `## 열린 항목` 중 자기 담당을 먼저 본다.
- 작업을 마치면 그 날 파일 `## 기록`에 항목을 덧붙이고(`### HH:MM [작업자] 제목`), 열린 항목을 갱신한다. 상대에게 원하는 것은 `요청 → Codex|Claude:`, 사용자 판단이 필요한 것은 `질문 → 사용자:`로 쓴다.
- `handoff/`는 **로컬 전용**(.gitignore)이다. 두 에이전트와 사용자가 같은 Mac 워킹트리를 쓰므로 커밋하지 않는다. 남겨야 할 결론은 코드 주석·커밋 메시지·`logs/`로 옮긴다.
